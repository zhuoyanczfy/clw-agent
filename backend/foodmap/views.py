...
# 22 张大阿卡纳：牌名 → {正位牌义, 逆位牌义}（含韦特牌面象征，凶牌保留警示性）
TAROT_CARDS = {
    '愚人': {'正位': '悬崖边的启程 · 天真无畏', '逆位': '鲁莽冒进 · 风险在前'},
    '魔术师': {'正位': '无限符号高悬 · 意念成真', '逆位': '口是心非 · 计划空转'},
    '女祭司': {'正位': '帐幔后的智慧 · 聆听直觉', '逆位': '忽视直觉 · 答案被自己挡住'},
    '女皇': {'正位': '丰饶花园 · 被爱滋养', '逆位': '过度付出 · 忘了疼自己'},
    '皇帝': {'正位': '石座王权 · 秩序与掌控', '逆位': '固执强势 · 失去弹性'},
    '教皇': {'正位': '传统指引 · 贵人相助', '逆位': '打破常规 · 走自己的路'},
    '恋人': {'正位': '天使祝福下的联结 · 心之所向', '逆位': '抉择摇摆 · 关系中的试探'},
    '战车': {'正位': '黑白马同驾 · 意志制胜', '逆位': '方向失控 · 欲速不达'},
    '力量': {'正位': '柔手驯狮 · 以柔克刚', '逆位': '自我怀疑 · 力量暂时沉睡'},
    '隐士': {'正位': '雪山孤灯 · 静中得慧', '逆位': '封闭自己 · 孤独感加深'},
    '命运之轮': {'正位': '轮盘转动 · 时来运转', '逆位': '时运暂滞 · 宜守不宜攻'},
    '正义': {'正位': '天秤与剑 · 因果分明', '逆位': '失衡偏颇 · 逃避决断'},
    '倒吊人': {'正位': '倒悬得悟 · 换个角度看', '逆位': '无谓牺牲 · 停滞不前'},
    '死神': {'正位': '白马过境 · 告别即新生', '逆位': '抗拒结束 · 困于旧局'},
    '节制': {'正位': '双杯活水 · 张弛有度', '逆位': '失衡过度 · 戒急戒躁'},
    '恶魔': {'正位': '锁链缠身 · 欲望的诱惑', '逆位': '挣脱束缚 · 觉察执念'},
    '高塔': {'正位': '闪电劈塔 · 旧局崩解', '逆位': '险中得缓 · 危机暗涌'},
    '星星': {'正位': '星空下注水 · 希望疗愈', '逆位': '微光摇曳 · 暂时迷茫'},
    '月亮': {'正位': '双犬吠月 · 情绪起伏', '逆位': '迷雾渐散 · 直面恐惧'},
    '太阳': {'正位': '向日葵与白马 · 光明喜乐', '逆位': '乌云蔽日 · 快乐打了折扣'},
    '审判': {'正位': '天使号角 · 觉醒召唤', '逆位': '错过召唤 · 旧事未了'},
    '世界': {'正位': '月桂花环 · 圆满达成', '逆位': '功亏一篑 · 差最后一步'},
}
        ...
        cards.append(
            {
                ...
                'keyword': TAROT_CARDS[name][orientation],
...
def _parse_reading(text, cards):
    ...
    if not lucky:
        ...
        future_card = cards[-1]
        lucky = f"今日宜顺着「{future_card['name']}」的指引：{future_card['keyword']}"
    ...
    messages = [
        {
            'role': 'system',
            'content': (
                '你是一位资深塔罗占卜师，精通韦特（Rider-Waite）塔罗体系，看牌极准。'
                '你的风格：神秘、笃定、直言牌意——好牌不吹捧，凶牌不回避，'
                '警示之后总会给出具体应对建议，因为来占卜的人是你很在意的人。'
                '禁止使用Markdown格式，直接输出纯文本。'
            ),
        },
        {
            'role': 'user',
            'content': (
                f'今天是{name}的每日三张牌占卜（时间之流牌阵：过去-现在-未来），牌面如下：\n'
                f'{card_lines}\n'
                '请以占卜师口吻为她解读今日运势，你的回答必须严格符合以下格式：\n'
                '只输出恰好两行：第一行以【今日解读】开头，第二行以【幸运指引】开头；'
                '不要有任何前言、后语、解释、编号或其他符号；两行顺序不可颠倒；'
                '标记符号只能用中文全角【】，不要用括号、冒号等其他符号代替。\n'
                '【今日解读】130~190字，占卜师摊开牌阵娓娓道来，硬性要求：\n'
                '1. 从牌面具体意象切入（韦特牌画面里的人物、动物、器物、颜色，'
                '如死神的白马与镰刀、高塔的闪电），再转到牌义；\n'
                '2. 逆位的牌必须按逆位牌义解读，绝不能当成正位说好话；\n'
                '3. 凶牌（死神、高塔、恶魔、月亮、塔罗逆位等）出现时直言警示：'
                '点明今天的坎在哪里、要避开什么，警示后紧跟一句具体的应对建议；\n'
                '4. 禁止“愿你被温柔以待”“美好会如约而至”这类万金油空话，'
                '每一句都要扣着这三张牌说；\n'
                '5. 结尾对未来位给出明确指向：是转机将至、需要等待、还是需要行动。\n'
                '【幸运指引】25字以内，从以下格式中选一种并写具体（不要泛泛的“保持好心情”）：'
                '幸运色+具体颜色名 / 幸运物+具体物品 / 今日宜+具体事项 / 今日忌+具体事项，'
                '如“今日宜尝一家没去过的新餐厅”“今日忌熬夜，23点前睡”。'
            ),
        },
    ]
    ...
    with transaction.atomic():
        ...
        try:
            ...
            reading = (
                f'牌面摊开：{names}。'
                '占卜师今日没有在线上，但牌已经替你说话——'
                '过去无法改写，现在正在你手里，未来尚未落笔。'
                '今天的解法只有一条：无论遇到什么，先深呼吸三次，再开口、再做决定。'
            )
            future_card = cards[-1]
            lucky = f"今日宜顺着「{future_card['name']}」的指引：{future_card['keyword']}"
